...
function getConfigHandler(req: Request, res: Response & { locals: OFDLocals }) {
    ...
    configurationStorage.retrieveUserConfiguration(res.locals.userId, configId as any).then(configRes => {
        configRes.map(config => {
            res.send(config);
        })
...
function saveConfigHandler(req: Request, res: Response & { locals: OFDLocals }) {
    const { configId } = req.params;
    const config: Configuration = {
        id: configId as any,
        value: req.body
    }
    configurationStorage.storeConfiguration(res.locals.userId, config).then(configRes => {
        configRes.map(() => {
            res.sendStatus(200);
        })
        .mapErr(err => storageErrorHandler(err, res))
