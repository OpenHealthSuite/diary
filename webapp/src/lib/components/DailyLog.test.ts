...
describe("Daily Log", () => {
    ...
    describe("Loading and Error logic", () => {
        ...
        it('it has loading indicator while loading, gone when resolved', async () => {
            ...
            (apiFetch as Mock<any[], any>).mockReturnValue(new Promise((resolve) => promiseResolve = resolve))
        ...
        it('it has loading indicator while loading, gone when rejected', async () => {
            ...
            (apiFetch as Mock<any[], any>).mockReturnValue(new Promise((resolve, reject) => promiseReject = reject))
            const { getByTestId, queryByTestId } = render(DailyLog, {
                day: testDate
            })
    ...
    describe("Data Display", () => {
        ...
        it('Shows the user the logs, in start time order, with calories', async () => {
            ...
            data.sort((a, b) => a.time.start.getTime() - b.time.start.getTime())
            .map(x => [x.name, x.metrics.calories]).forEach(([name, calcount], i) => {
